    ...
    <div class="operationWords">
        ...
        <div v-if="userInfo && userObj.checked" class="flex">
            <div class="operation_btn">
                ...
                <el-card v-for="(o, index) in trickList" :key="o.id"
            ...
            <div id="NodeRelation" style="flex: 1">
                <div ref="plumb_header" class="plumb_header flex">
                    ...
                    <el-button type="primary" style="margin: 0 20px" :icon="CollectionTag" @click="preserve()">保存
                    </el-button>
                ...
                <div class="diagramContainer-warp">
                    <div class="diagramContainer_content" @mousemove="mousemoveHandler">
                        <div v-if="list.length != 0" id="diagramContainer">
                            <Node v-for="item in list" :key="item.id" :scenarios-info="scenariosInfoFn"
                                :is-preserve="isPreserve" :data-obj="item" :token="updatatoken"
                                @mouseup="mouseup(item.id)" @get-info="getInfo" @del-node-fn="delNodeFn" />
                        </div>
                    </div>
                </div>
                <div class="move_view">
                    <div style="width:100%;height: 100%;position: relative;" class="move_lookWarp">
                        ...
                        <span v-for="item in list" :id="`move_span${item.id}`" :key="item.id" class="dian"
                            :style="`top:${positionXY(item.position).x}px;left:${positionXY(item.position).y}px ;`"></span>
                    </div>
                </div>
...
function move_view() {
    ...
    lookView.style.cssText += `width:${warp.clientWidth / 50}px;height:${warp.clientHeight / 50}px;top:${3000 / 50}px;left:${3000 / 50}px;`
    if (!window.onresize) {
...
function drog() {
    ...
    content.addEventListener('mousedown', (e: MouseEvent) => {
        ...
        document.onmousemove = function (e) {
            ...
            e = e || window.event;
            const left = Math.max(
                Math.min(e.clientX - offsetX, 0),
                -(content.clientWidth - warp.clientWidth),
            )
            const top = Math.max(
    ...
    lookView.addEventListener('mousedown', (e: MouseEvent) => {
        ...
        document.onmousemove = function (e) {
            ...
            const left = Math.min(
                ...
                (lookWarp.clientWidth - lookView.clientWidth),
            )
            const top = Math.min(
                Math.max(e.clientY - offsetY, 0),
                (lookWarp.clientHeight - lookView.clientHeight),
            )
